...
TEST(test_buffer, hip_memcpy1) {
  ...
  hipMemcpy(ptr2, cu_buffer.ptr(), sizeof(float) * size, hipMemcpyDeviceToHost);
  for (int i = 0; i < size; ++i) {
    // ptr[i] = float(i);
...
TEST(test_buffer, hip_memcpy3) {
  ...
  // cu to cu
  ASSERT_EQ(cu_buffer1.device_type(), DeviceType::kDeviceHIP);
  ASSERT_EQ(cu_buffer2.device_type(), DeviceType::kDeviceHIP);

  cu_buffer1.copy_from(cu_buffer2);

  float* ptr2 = new float[size];
  hipMemcpy(ptr2, cu_buffer1.ptr(), sizeof(float) * size, hipMemcpyDeviceToHost);
  for (int i = 0; i < size; ++i) {
